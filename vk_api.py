import requests
import ujson
import pickle
import progressbar

OWNER_ID = '-86678270'
API_URL = 'https://api.vk.com/method/'
WALL_GET_METHOD = 'wall.get'
USER_INFO_METHOD = 'users.get'
CITY_GET_METHOD = 'database.getCitiesById'
COMMENT_GET_METHOD = 'wall.getComments'
API_VERSION = '5.63'
DEFAULT_MESSAGE = 'Not available'
bar = progressbar.ProgressBar()


def get_resp_with_method(api_url, method, params):
    api_request_url = api_url + method
    response = requests.get(api_request_url, params)
    response_dic = ujson.loads(response.text)
    return response_dic


class VkGroup(object):
    posts = list()

    def download_posts(self):
        wall_get_params = {'owner_id': self.id, 'count': '100', 'v': API_VERSION, 'offset': 0}
        while True:
            response_dic = get_resp_with_method(API_URL, WALL_GET_METHOD, wall_get_params)
            posts_dic = response_dic['response']['items']
            if len(posts_dic) < 1 :
                break
            for elem in bar(posts_dic):
                self.posts.append(Post(elem))
            wall_get_params['offset'] += 100

    def __init__(self, owner_id):
        self.id = owner_id
        self.download_posts()


class User(object):

    def __init__(self, id):
        self.id = id
        self.age = DEFAULT_MESSAGE
        self.city = DEFAULT_MESSAGE
        self.get_user_info()

    def get_user_info(self):
        user_params = {'user_ids': self.id, 'fields': 'city,bdate', 'v': API_VERSION}
        vk_structure = get_resp_with_method(API_URL, USER_INFO_METHOD, user_params)
        vk_structure = vk_structure['response'][0]
        try:
            date_struct = vk_structure['bdate'].split('.')
            date_len = len(date_struct)
            if date_len == 3:
                self.age = date_struct[2]

        except KeyError:
            pass
        except IndexError:
            pass
        try:
            self.city = vk_structure['city']['title']
        except KeyError:
            pass

class Comment(object):

    def __init__(self, vk_structure):
        self.text = vk_structure['text']
        self.comm_id = vk_structure['id']
        self.owner = User(vk_structure['from_id'])


class Post(object):
    comments = list()

    def __init__(self, vk_structure):
        self.text = vk_structure['text']
        self.post_id = vk_structure['id']
        self.owner_id = vk_structure['owner_id']
        try:
            self.signer = User(vk_structure['signer_id'])
        except KeyError:
            self.signer = DEFAULT_MESSAGE
        self.get_comments()

    def get_comments(self):
        comments_get_params = {'owner_id': self.owner_id, 'post_id': self.post_id, 'count': '100', 'v': API_VERSION, 'offset': 0}
        while True:
            response_dic = get_resp_with_method(API_URL, COMMENT_GET_METHOD, comments_get_params)
            # First element is count of posts, so we exclude it
            comments_dic = response_dic['response']['items']
            if len(comments_dic) < 1:
                break
            for elem in comments_dic:
                self.comments.append(Comment(elem))
            comments_get_params['offset'] += 100



if __name__ == '__main__':
    cool_shit = VkGroup(OWNER_ID)
    with open('cool_shit.pkl', 'wb') as f:
        pickle.dump(cool_shit, f)